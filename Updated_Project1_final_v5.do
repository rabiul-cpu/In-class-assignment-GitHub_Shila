...
* Set working directory - change this to the own folder path
cd "C:\Users\u1592799\Box\0. Utah\Course works & TA\2nd sem_SP26\D.JVan\Project 1 updated\0 .Submitted"
...
log using "Final_ansur_log.txt", replace text
...
tab gender, missing
tab component, missing
tab branch, missing
tab writingpreference, missing

**no missing*

* Check race variables — verify values before any cleaning
* SubjectNumericRace: multi-race allowed (1-6, 8)
* DODRace: single preferred race (1-6, 8)
tab subjectnumericrace, missing
tab dodrace, missing

* Check string demographic variables — no cleaning 
* Ethnicity: self-reported e.g. "Mexican", "Vietnamese"
* Installation: Army base where measured e.g. "Fort Hood"
tab ethnicity, missing
tab installation, missing

* Check the date variable
* I need to know how many observations have missing dates
codebook date
tab date if date == .
...
That duplicate stays in the dataset
...
* Verify: one obs should now have .c across all anthro vars
di _newline "Observations with stature = .c (unknown missing):"
count if stature == .c
/*. count if stature == .c
  1
So, I have successfully converted the missing values to stata MV
  */
...
*
* WHY earlier?
* - I remove duplicates first because there is no reason to fix data that i will delete anyway
* - I will delete these observations anyway
* - It saves time to delete them earlier
* - Then I only clean the observations i actually keep. Every valid measurement has a valid date. 
*--------------------------------------------------------------------------

/*

Using standard stata approach-duplication deletion

*  check for exact duplicates across all variables
duplicates report

* tag them. Tag duplicate groups to review them
duplicates tag, generate(dup_count)

* create your flag variable (keeps your structure)
gen flag_duplicate = (dup_count > 0)
label variable flag_duplicate "1 = potential duplicate observation"
...
preserve
    ...
    save "Islam_Ansur_duplicates.dta", replace
restore

* remove duplicates, keep one of each group
duplicates drop
...
--------------+-----------------------------------
Not duplicate |      7,031      100.00      100.00
--------------+-----------------------------------
        Total |      7,031      100.00
...
**All .c values are located in duplicate records and are removed when duplicates were dropped

*[preserve // make a photocopy (Take a snapshot of the full dataset) and put it aside safely. Take a snapshot of the full dataset. Stata memorizes everything at this point. 
...
*restore // Go back to the full dataset i saved in preserve. Everything is back to normal - nothing was permanently changed
...
*Exact-match deduplication using Stata's duplicates report identified zero duplicate records, as duplicate rows differ from their originals only in the date variable.  the date variable records the participant's measurement date ranging from 04-Oct-10 to 05-Apr-12, and every genuine survey observation carries a valid date. Accordingly, 963 observations with missing measurement dates (date == .) identified as duplicate records, flagged, saved separately, and removed from the analytic dataset, leaving 6,068 observations.

Every participant was measured on a specific date within that range. A missing date therefore indicates the record is not a genuine independent observation. 


Before using any deduplication method, the normal exact-match method (duplicates report) was used on all variables. This found no duplicates because the only difference between duplicate rows in this dataset is the date variable: one row has a valid measurement date and the other has date == . This makes them look unique when compared to each other, even though they are both for the same participant.


I did not use exact-match deduplication on demographic and anthropometric information. This was turned down because it assumes that no two genuine armies can have the same values for all the factors that were measured, which is not a valid premise to me. There are 93 measurements taken to the closest millimeter, therefore precise ties are very likely, especially for frequent round-number values. It is also possible for two armies to really be the same age, height, weight, race, branch, and component. I did not see their birth data as well. If I flag these situations as duplicates and delete them, I can accidentally delete legitimate participants from the dataset, which would create selection bias without a good reason.

The second option was the normal Stata duplicates drop command, which employed exact-match logic on all variables. As stated above, this found no duplicates, hence it didn't apply to this dataset.

The method that was chosen used the date variable to find duplicates. According to the ANSUR II Memorandum for Record (Paquette & Parham, 2017, p. 1), the date is the specific session date on which each participant was measured. All valid observations were made between 04 October 2010 and 05 April 2012. Because the survey was an organized, in-person measurement session, every real observation must have a measurement date that goes with it. An observation with date == . cannot signify an independent data gathering event and was consequently regarded as a duplicate record. This method is based on the survey's administrative structure. It is also conservative in that it only excludes observations that have a clear and documented cause for being excluded.

The result of this procedure confirmed its validity. After taking out the 963 observations that didn't have a date, the analytic sample had exactly 6,068 observations. This matched the total recorded sample of 4,082 males and 1,986 females given on page 1, paragraph 3 of the ANSUR II MFR (4,082 + 1,986 = 6,068; Paquette & Parham, 2017). This exact match with the official records shows that the observations that were eliminated were administrative duplicates and not real survey participants. It also shows that no real data were lost in the process.

One drawback of this method is that it can't find circumstances when a duplicate record was accidentally given a fake or copied date. But since the sample sizes match exactly, it's unlikely that this happened in this dataset. The duplicate data are saved and uploaded to Canvas. 
...
* Verify
count
* 6066 observations in the dataset
...
* - I can now check which real values are implausible
* - I create a flag variable to document all problems found
*
* FLAG VARIABLE RULES:
*   0 = no problem found
...
* I only flag and remove impossible values
...
tab flag_suspect, missing
tab heightin, missing
*---------------------------------------------------------------------------
* --- FIX subjectnumericrace ---
* Self-reported race — multiple races allowed
* Valid codes: 1=White 2=Black 3=Hispanic 4=Asian
*              5=Native American 6=Pacific Islander 8=Other
* Code 7 is NOT defined in codebook — treat as suspect
* Any value outside 1-6 and 8 is implausible
*---------------------------------------------------------------------------
tab subjectnumericrace, missing

replace flag_suspect = 1 if                                          ///
    !inlist(subjectnumericrace, 1, 2, 3, 4, 5, 6, 8)                ///
    & subjectnumericrace != .
replace subjectnumericrace = .d if                                   ///
    !inlist(subjectnumericrace, 1, 2, 3, 4, 5, 6, 8)                ///
    & subjectnumericrace != .
...
*---------------------------------------------------------------------------
* --- CHECK DODRace ---
* Single preferred race — DoD manpower comparable
* Valid codes: 1=White 2=Black 3=Hispanic 4=Asian
*              5=Native American 6=Pacific Islander 8=Other
* Verified by user: values only 1-6 and 8 — no suspect values
* Therefore: no flagging needed — label only in Step 3g
*---------------------------------------------------------------------------
tab dodrace, missing
...
* - If i make a mistake i can always go back to the original
* - i can compare old and new values to verify the conversion
...
*
* WHY DO THIS?
* - When i created new cm variables by dividing by 10
* - Stata did NOT automatically copy the missing codes
* - I must manually transfer them from original to new variables
*
* I transfer ALL missing types:
*   .a = not recorded (-77)
*   .b = refused (-88)
*   .c = unknown (-99)
*   .d = decided missing (suspect values i set to missing)
*---------------------------------------------------------------------------
...
* Label all new cm variables so i know what they are
...
* - i can now check plausible ranges in familiar cm units
* - Based on US Army standards and observed data distributions
* - i add to the existing flag_suspect variable
*
* RANGES BASED ON:
...
* --- STATURE (HEIGHT) --- *stature /standing height. It is the vertical distance from the floor to the top of the head — essentially the standing height of the soldier measured directly with a stadiometer while standing erect. 
...
* Should be about 85-88% of stature. *cervicaleheight – Cervical Height //It is the vertical distance from the floor to the most prominent point of the seventh cervical vertebra at the base of the neck — measured while a person stands erect. Cervicale height is typically about 85–88% of total stature. So if stature is ~172 cm, cervicale height should be roughly ~146–151 cm. This is the most strongly correlated measure with stature (r=0.991). A big difference from this ratio is highly suspicious.
...
replace _cervicaleheight_cm = .d if _cervicaleheight_cm < 110 & _cervicaleheight_cm != .
replace _cervicaleheight_cm = .d if _cervicaleheight_cm > 180 & _cervicaleheight_cm != .

* --- TROCHANTERION HEIGHT ---*trochanterionheight – Trochanterion Height //It is the vertical distance between the standing surface and the trochanterion landmark on the upper side of the right thigh — measured with an anthropometer while the subject stands erect looking straight ahead with heels together and weight distributed equally on both feet
*  range: 60 to 115 cm
...
replace _trochanterionheight_cm = .d if _trochanterionheight_cm < 60  & _trochanterionheight_cm != .
replace _trochanterionheight_cm = .d if _trochanterionheight_cm > 115 & _trochanterionheight_cm != .

* --- WAIST HEIGHT OMPHALION --- *Waist height omphalion is typically about 58–62% of total stature So if stature is ~172 cm, waist height should be roughly ~100–107 cm. A big difference from this ratio is suspicious. I will consider 80-130 for safe range.anything beyon this range is suspecious. Obseeved range is Range: [80.5,124.5].  I see no observation in suspecious range. 
*  range: 80 to 130 cm
...
replace _waistheightomphalion_cm = .d if _waistheightomphalion_cm < 80  & _waistheightomphalion_cm != .
replace _waistheightomphalion_cm = .d if _waistheightomphalion_cm > 130 & _waistheightomphalion_cm != .

* --- KNEE HEIGHT MIDPATELLA --- *kneeheightmidpatella – Knee Height, Midpatella //It is the vertical distance from the floor to the midpoint of the kneecap (patella) while someone stands erect with feet slightly apart and weight distributed equally. *I will use value less than 38 as suspecious. It is too short. 38 to 63 cm as my plausible range for kneeheightmidpatella- knee height should be approximately 28–30% of total height. I found some values below 38 as suspecious. Yes!!!!!!!!!!!!!
* Plausible range: 38 to 63 cm
* Knee height should be approximately 28-30% of total stature
* Values below 38 cm are too short to be a real adult knee height
* Values above 63 cm are too tall — physiologically impossible
...
replace _kneeheightmidpatella_cm = .d if _kneeheightmidpatella_cm < 38 & _kneeheightmidpatella_cm != .
replace _kneeheightmidpatella_cm = .d if _kneeheightmidpatella_cm > 63 & _kneeheightmidpatella_cm != .

* --- FUNCTIONAL LEG LENGTH ---*functionalleglength – Functional Leg Length /It is the straight-line distance from the base of the foot to the back surface of the body in line with the trochanter, measured with the subject sitting and right leg extended.
...
replace _functionalleglength_cm = .d if _functionalleglength_cm < 78  & _functionalleglength_cm != .
replace _functionalleglength_cm = .d if _functionalleglength_cm > 138 & _functionalleglength_cm != .

*I see the range is [83.5,131.6 cm]. Functional leg length is typically about 60–68% of total stature. So if stature is ~172 cm, functional leg length should be roughly ~103–117 cm. A big difference from this ratio is suspicious
* I will use 78 to 138 cm as your plausible range for functionalleglength and always cross check against stature — functional leg length should be approximately 60–68% of total height. So, there is no suspecious values. 



* --- FOOT LENGTH --- * I will use 19 to 34 cm as my plausible range for footlength based on ANSUR II data collected from US Army Active Duty, National Guard, and Army Reserve Soldiers. I see clearly that there is no suspected values. 
...
replace _footlength_cm = .d if _footlength_cm < 19 & _footlength_cm != .
replace _footlength_cm = .d if _footlength_cm > 34 & _footlength_cm != .

* --- THUMBTIP REACH --- /* I will use 55 to 105 cm as my plausible range for thumbtipreach. The values above 200 cm in my dataset are clearly suspected or out of ranege. YES!!!!!!
*/

* Plausible range: 55 to 105 cm
* Human thumbtip reach cannot physically exceed 105 cm
* Values below 55 cm are also impossible for an adult soldier
...
replace _thumbtipreach_cm = .d if _thumbtipreach_cm < 55  & _thumbtipreach_cm != .
replace _thumbtipreach_cm = .d if _thumbtipreach_cm > 105 & _thumbtipreach_cm != .

* --- SPAN (ARM WINGSPAN) --- * Span (distance between the tips of the right and left middle fingers when the subject's arms are stretched out horizontally to both sides).
...
replace _span_cm = .d if _span_cm < 132 & _span_cm != .
replace _span_cm = .d if _span_cm > 215 & _span_cm != .

tab _span_cm, missing


* --- CHEST CIRCUMFERENCE --- *looks ok. no suspecious value. Some values looks small but female officer might have it physiologicaly possible. 
...
replace _chestcircumference_cm = .d if _chestcircumference_cm < 65  & _chestcircumference_cm != .
replace _chestcircumference_cm = .d if _chestcircumference_cm > 150 & _chestcircumference_cm != .

* --- WAIST CIRCUMFERENCE --- *distribution looks good, no suspecious value
...
replace _waistcircumference_cm = .d if _waistcircumference_cm < 55  & _waistcircumference_cm != .
replace _waistcircumference_cm = .d if _waistcircumference_cm > 140 & _waistcircumference_cm != .

* --- HIP BREADTH --- Looks ok.
...
replace _hipbreadth_cm = .d if _hipbreadth_cm < 25 & _hipbreadth_cm != .
replace _hipbreadth_cm = .d if _hipbreadth_cm > 50 & _hipbreadth_cm != .
...
replace _hipbreadthsitting_cm = .d if _hipbreadthsitting_cm < 25 & _hipbreadthsitting_cm != .
replace _hipbreadthsitting_cm = .d if _hipbreadthsitting_cm > 58 & _hipbreadthsitting_cm != .

* --- BICRISTAL BREADTH --- *It is the horizontal distance between the left and right iliocristale landmarks — the widest points of the pelvis (hip bones) — measured while the soldier stands erect
...
replace _bicristalbreadth_cm = .d if _bicristalbreadth_cm < 17 & _bicristalbreadth_cm != .
replace _bicristalbreadth_cm = .d if _bicristalbreadth_cm > 40 & _bicristalbreadth_cm != .
...
*
* WHY LABEL?
* - Labels make the dataset professional and easy to understand
...
*---------------------------------------------------------------------------
* --- LABEL SubjectNumericRace ---
* Already numeric — add value labels so output shows text not numbers
* Same valid codes as DODRace but allows multiple race selection
*---------------------------------------------------------------------------
label define race_lbl 1 "White"            ///
                      ...
                      6 "Pacific Islander" ///
                      8 "Other"
label values subjectnumericrace race_lbl
label variable subjectnumericrace ///
    "Self-reported race — multiple races allowed (codes 1-6, 8)"
tab subjectnumericrace, missing

*---------------------------------------------------------------------------
* --- LABEL DODRace ---
* Already numeric — verified clean by user, no flagging needed
* Reuse race_lbl defined above — same valid codes
*---------------------------------------------------------------------------
label values dodrace race_lbl
label variable dodrace ///
    "Single preferred race — DoD manpower comparable (codes 1-6, 8)"
tab dodrace, missing

*---------------------------------------------------------------------------
* --- LABEL ethnicity ---
* String variable — self-reported e.g. "Mexican", "Vietnamese"
* Verified by user: no missing, no suspect values
* No cleaning needed — label only
*---------------------------------------------------------------------------
label variable ethnicity ///
    "Self-reported ethnicity (string) — verified through interview"
tab ethnicity, missing

*---------------------------------------------------------------------------
* --- LABEL installation ---
* String variable — Army base where measurement occurred
* e.g. "Fort Hood", "Camp Shelby"
* Verified by user: no missing, no suspect values
* No cleaning needed — label only
*---------------------------------------------------------------------------
label variable installation ///
    "U.S. Army installation where measurement occurred (string)"
tab installation, missing
...
* Transfer .d from source variables to bmi and bmi_cat
*No new flag needed — already flagged in Step 3d. Just transfer the .d so it shows correctly
* Stata does not carry .d through calculations automatically. i must do this manually
replace bmi     = .d if weightkg == .d | _stature_cm == .d
replace bmi_cat = .d if weightkg == .d | _stature_cm == .d
...
* i extract the month from the date variable first
...
/*date is a Stata date variable — it stores the full date
For example: 15 March 2012
month() is a Stata function that extracts just the month number
So March → 3
*/
...
* Rationale: i use stature and BMI to define body types. Applied separately for males and females  because height distributions differ between sexes. 
*
...
/*
"Body type was classified using sex-specific stature percentiles
from the ANSUR II sample combined with WHO BMI cutpoints.
Height categories were defined separately for males and females
because height distributions differ significantly between sexes.
The Tall category was further divided into Tall-Lean (BMI < 25)
and Tall-Heavy (BMI ≥ 25) to capture both size dimensions."
*/
...
/*
"Age is categorized using the National Center for Health
Statistics (NCHS) standard age groups: 18–24, 25–44, 45–64,
and 65 years and older. These categories are widely used in
US health research to enable comparison with national
population data."
*/
tab age, missing
gen age_cat = .
...
replace hip_height_ratio = .d if _trochanterionheight_cm == .d | _stature_cm == .d
label variable hip_height_ratio "Trochanterion height as % of stature"

* Check by sex
bysort gender: summarize hip_height_ratio, detail
...
replace heightin_cm = .d if heightin == .d
...
replace weightlbs_kg = .d if weightlbs == .d
...
replace height_diff = .d if heightin_cm == .d | _stature_cm == .d
gen weight_diff = weightlbs_kg - weightkg
replace weight_diff = .d if weightlbs_kg == .d | weightkg == .d
...
/****************************************************************
   ...
   5   - BMI and demographics (For me: 7101 students)
****************************************************************/

*---------------------------------------------------------------------------
* SECTION 3.2 - DATA QUALITY ASSESSMENT SUMMARY
* Produces counts used to write the data quality paragraph in report
*---------------------------------------------------------------------------

* Total observations at each cleaning stage
di "Original observations: 7,031"
di "After removing duplicates (date==.): 6,068"
di "After removing under-18: 6,066"

* Final count of suspect values across all variables
tab flag_suspect



* How many observations have at least one suspect value
count if flag_suspect == 1
di "Observations with at least one suspect value: " r(N)

* Missing value summary across all analytic variables after cleaning
misstable summarize _stature_cm _cervicaleheight_cm          ///
    _trochanterionheight_cm _waistheightomphalion_cm          ///
    _kneeheightmidpatella_cm _functionalleglength_cm          ///
    _footlength_cm _thumbtipreach_cm _span_cm                 ///
...
* Note on weightkg: codebook states kg*10 but inspection shows values
* consistent with direct kg (mean ~82 kg). No conversion applied.
summarize weightkg, detail
...
* Check the distribution of the suspect and duplicate flags
tabulate flag_suspect
tabulate possible_dup_flag

* Verify final age range
summarize age
...
*Table 4.1.a: Assess the level of correlation between the measures of stature
	
pwcorr _stature_cm _cervicaleheight_cm _trochanterionheight_cm ///
       _waistheightomphalion_cm _kneeheightmidpatella_cm ///
       _functionalleglength_cm _footlength_cm ///
       _thumbtipreach_cm _span_cm, sig star(0.05)
...
twoway (histogram weight_diff if gender == "Female",  ///
        fcolor(red%40) lcolor(red%40) width(5))     ///
       (histogram weight_diff if gender == "Male",    ///
        fcolor(navy%40) lcolor(navy%40) width(5)),  ///
       legend(order(1 "Female" 2 "Male"))             ///
       xtitle("Reported minus Measured Weight (kg)")  ///
...
twoway (scatter bmi age if gender == "Female",           ///
       ...
       xtitle("Age (years)") ytitle("BMI (kg/m^2)")       ///
...
*---------------------------------------------------------------------------
* SECTION 5c - COMPARISON OF CONTINUOUS VS CATEGORICAL AGE ANALYSIS
* This is a written comparison — the code below documents the statistics
* used to support the comparison paragraph in the report
*---------------------------------------------------------------------------

* Continuous analysis (5a): Pearson r values already computed above
* Recall: Female r ~ 0.10-0.15, Male r ~ 0.15-0.20 (weak positive)

* Categorical analysis (5b): Mean BMI per age group shows clearer pattern
bysort gender age_cat: summarize bmi
* The categorical approach reveals which specific age groups differ most
* e.g. 25-44 vs 45-64 differences are hidden in a single correlation coefficient

* Proportion obese by age group and sex — shows practical significance
bysort gender age_cat: tab bmi_cat
...
/***************************************************************
   ...
   WHY SAVE AT THE END?
   - I save the final clean dataset with all new variables
   - This is the analytic dataset i submit with our report
...
save "ANSUR_analytic_dataset_Islam26.dta", replace
